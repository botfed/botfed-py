...
"""Copy of Info from hyperliquid sdk, but using our monkey patched websocket manager to auto reconnect"""
...
class Info(API):
    ...
    def post_ws(self, data):
        if self.ws_manager is None:
            raise RuntimeError("Cannot call post_ws since skip_ws was used")
        else:
            return self.ws_manager.post(data)
    ...
    def bulk_subscribe(self, subs: List[Tuple[Subscription, Callable[[Any], None]]]) -> int:
        if self.ws_manager is None:
            raise RuntimeError("Cannot call subscribe since skip_ws was used")
        else:
            for subscription, callback in subs:
                self.ws_manager.add_subscription(subscription, callback)
            return self.ws_manager.subscribe_queued()
    ...
    def subscribe_post_ws(self, callback) -> int:
        if self.ws_manager is None:
            raise RuntimeError("Cannot call subscribe_post_ws since skip_ws was used")
        else:
            return self.ws_manager.subscribe_post_ws(callback)
